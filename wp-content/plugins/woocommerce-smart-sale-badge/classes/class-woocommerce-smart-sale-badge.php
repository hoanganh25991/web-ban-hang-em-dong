...
class WooCommerce_Smart_Sale_Badge {
	...
	public function badge_sale_amount( $message, $post, $product ) {
		...
		if ( $product->has_child() ) {
			...
			$button_text = apply_filters("wc_smart_sale_badge_title", __( 'Tiết kiệm', 'wc_smart_sale_badge' ), $product->has_child(), $product);
			...
			$button_text = apply_filters("wc_smart_sale_badge_title", __( 'Giảm giá', 'wc_smart_sale_badge' ), $product->has_child(), $product);
